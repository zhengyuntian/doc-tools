...
### 九、PDF图片/图表检测问题修复 ✅
- [x] 标注重复问题修复（Set去重机制，规则编码+类型+页码+索引唯一键）
- [x] 身份证图片误判为表格修复（宽高比+纹理线条+圆角特征排除）
- [x] 非表格图片误判修复（线条检测阈值优化，hLines/vLines>30才判定为表格）
- [x] 表格图片未识别修复（优化表格检测逻辑）
- [x] PDF最后几页图片未提取修复（注释图片提取，处理Widget类型注释外观流）
- [x] 所有规则数量限制移除（移除results.size() >= 10限制）
- [x] 数据库字段长度问题修复（TEXT改为LONGTEXT，支持大数据存储）
